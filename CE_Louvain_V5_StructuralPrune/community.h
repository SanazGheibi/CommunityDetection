...
class Community {
 public:
  ...
  int max_degree; //used for bucket sort
  int max_batchSize; //for initializing the queue size
  int total_size; //used to distinguish between the original graph and the pruned one
  int boundary;
  ...
  // type defined the weighted/unweighted status of the graph file. "iter1" added by sanaz
  Community (char *filename, char *filename_w, int type, int nb_pass, double min_modularity, bool iter1);
  // copy graph
  ...
  bool one_level(int W, bool level0);
  
  //sanaz:
  int size_estimate(int start, long W);
  void ABFS_preorder(vector<int>& retorder, long W);
  void transform(long W);
