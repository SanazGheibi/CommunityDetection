...
Community::miniBatch_assign(long W){
   ...
   while(start < size){
   	long sum = 0;
   	int index = start;
   	int max_deg = 0; //neigh_pos[] will grow as large as the largest degree in the node group
   	int diff = 0;
   	while(index < size && sum < W){
      		...
      		long tmp = sizeof(int) + sizeof(unsigned long) + sizeof(double) + deg * sizeof(unsigned int); //Accounting for n2c, g.degrees[], neigh_weight[] and g.links[] 
		if(g.weights.size() != 0)
