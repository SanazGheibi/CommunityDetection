...
//sanaz: function added by me
Graph
Community::reorder() {
  // Renumber communities
  // Mark the communities that do exist
  vector<int> renumber(size, -1);
  for (int node=0 ; node<size ; node++) {
    renumber[n2c[node]]++;
  }
  ...
  //renumber[i] shows the new community ID for community i 
  ...
  g2.degrees.resize(g.nb_nodes);

  int comm_deg = comm_nodes.size();
  int index = 0; //used to indexing the nodes
 
  //first compute new indices for nodes
  vector<int> node_renumber(size, -1);
  for (int comm=0 ; comm<comm_deg ; comm++) {
    int comm_size = comm_nodes[comm].size();

    // Iterate over members of communities
    for (int tmp_node=0 ; tmp_node<comm_size ; tmp_node++) {
         int node = comm_nodes[comm][tmp_node];
         node_renumber[node] = index;
         index++;
    }
  } 

  g2.weights.resize(g.weights.size());
  ...
  for (int comm=0 ; comm<comm_deg ; comm++) {
    ...
    for (int node=0 ; node<comm_size ; node++) {
      ...
      for (int i=0 ; i<deg ; i++) {
	int neigh        = *(p.first+i);
	g2.links.push_back(node_renumber[neigh]);
        if(g.weights.size()!=0)
...
bool
Community::one_level(int orderIter) {
  //sanaz: input orderIter tells us before which iteration to do the reordering
  //iteration numbers start from 0. orderIter = 1 makes us do the reordering before the second iteration
  ...
  do {
    ...
    if(iter == orderIter){
      ...
      //g.display2();
      g = reorder();//call the reordering function
      for (int i=0 ; i<size ; i++)
          random_order[i]=i;
      //cerr << "new graph:" << endl;
      //g.display2();
