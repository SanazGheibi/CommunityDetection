...
echo -n "" > bc.txt
./community ~/Louvain_input/uscOrdered_amazon.bin -l -1 -v  -q 0.0001 > graph.tree 2> res.txt
...
for((i=1; i < $n; i++)); do
    echo "i: $i"
    ./community ~/Louvain_input/uscOrdered_amazon.bin -p comm.txt -l -1 -v -q 0.0001 > graph.tree 2> res.txt
    grep 'duration' ./res.txt >> runtimes.txt
    grep 'lastLevel' ./res.txt > level_info.txt
    read -r pass level < level_info.txt
    ./hierarchy graph.tree -l $level > comm.txt
